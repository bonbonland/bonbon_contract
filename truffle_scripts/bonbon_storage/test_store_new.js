module.exports = async function (done) {
  const TestCaller = artifacts.require('./bonbon_storage/TestCallerNew')
  const DdzShareAccount = artifacts.require('./bonbon_storage/DdzShareAccount')
  const BonBonStorage = artifacts.require('./bonbon_storage/BonBonStorage')
  ...
  try {
    ...
    DeployedStorage = await BonBonStorage.deployed()
    abi = DeployedTestCaller.contract.abi
    web3 = new Web3(config.envConfig.DEV_CHAIN_HTTP_HOST)
    currentDdzShareAddount = await DeployedStorage.getShareAccount.call(gameName) //这里调用的是方法，有modifier
    ...
    let setShareAccount = async function () {
      await DeployedStorage.setShareAccount(gameName, '0x8fde55b27790902de6cf5ad66a1dfd789a7f6ddb', {
